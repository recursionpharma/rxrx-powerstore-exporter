...
func NewWearMetricCollector(api *client.Client, bulkApi *bulkClient.BulkClient, logger log.Logger) *metricWearMetricCollector {
...
func (c *metricWearMetricCollector) Collect(ch chan<- prometheus.Metric) {
	...
	if c.isEnableBulk {
		...
		for _, data := range driveDataJson.Array() {
			...
			driveID := data.Get("drive_id").String()
			driveName := driveArray["drive"][driveID]
			metricsValue := data.Get("percent_endurance_remaining")
			metricDesc := c.metrics["wear"]
			if metricsValue.Exists() && metricsValue.Type != gjson.Null {
				ch <- prometheus.MustNewConstMetric(metricDesc, prometheus.GaugeValue, metricsValue.Float(), driveName.String(), applianceID)
			}
		}
	} else {
